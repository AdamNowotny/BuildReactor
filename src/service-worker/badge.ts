...
const init = () => {
    ...
    stateStorage.onChanged.subscribe(({ newValue }) => {
        ...
        const failedCount = newValue.reduce((acc, item) => acc + (item.failedCount ?? 0), 0);
        const runningCount = newValue.reduce((acc, item) => acc + (item.runningCount ?? 0), 0);
        const offlineCount = newValue.reduce((acc, item) => acc + (item.offlineCount ?? 0), 0);
        if (runningCount > 0) {
